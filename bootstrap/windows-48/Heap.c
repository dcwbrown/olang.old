/* voc  Oberon compiler olang 0.5 [2016/03/14] for cygwin ILP32 using gcc tskSF */
#include "SYSTEM.h"
...
SYSTEM_PTR Heap_NEWBLK (LONGINT size)
{
	...
	tag = (__VALP(LONGINT, new) + blksz) - 24;
	__PUT(tag - 8, 0, LONGINT);
	__PUT(tag, blksz, LONGINT);
	__PUT(tag + 8, -8, LONGINT);
	__PUT(__VALP(LONGINT, new) - 8, tag, LONGINT);
	Heap_Unlock();
	_o_result = new;
	return _o_result;
}
		...
		if (!__ODD(tagbits)) {
			...
			for (;;) {
				...
				if (offset < 0) {
					...
					__PUT(fld, __VALP(SYSTEM_PTR, n), SYSTEM_PTR);
				} else {
					fld = q + offset;
					...
					if (n != 0) {
						...
						if (!__ODD(tagbits)) {
							...
							__PUT(fld, __VALP(SYSTEM_PTR, p), SYSTEM_PTR);
							p = q;
							q = n;
							tag = tagbits;
						}
...
static void Heap_Finalize (void)
{
	...
	while (n != NIL) {
		if (!n->marked) {
			...
			(*n->finalize)(__VALP(SYSTEM_PTR, n->obj));
			if (prev == NIL) {
				n = Heap_fin;
			} else {
				n = n->next;
...
__TDESC(Heap_ModuleDesc, 1, 2) = {__TDFLDS("ModuleDesc", 72), {0, 32, -24}};
__TDESC(Heap_CmdDesc, 1, 1) = {__TDFLDS("CmdDesc", 32), {0, -16}};
__TDESC(Heap_FinDesc, 1, 1) = {__TDFLDS("FinDesc", 24), {0, -16}};
__TDESC(Heap__1, 1, 1) = {__TDFLDS("", 8), {4, -16}};
