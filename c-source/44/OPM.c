...
BOOLEAN OPM_OpenPar (void)
{
	...
	if (Platform_ArgCount == 1) {
		...
		Console_Ln();
		Console_String((CHAR*)"  M - generate code for main module and link object statically", (LONGINT)63);
		Console_Ln();
		...
		Console_Ln();
		Console_String((CHAR*)"  x - turn off array indices check", (LONGINT)35);
		Console_Ln();
		Console_String((CHAR*)"  a - don\'t check ASSERTs at runtime, use this option in tested production code", (LONGINT)80);
		Console_Ln();
		Console_String((CHAR*)"  p - turn off automatic pointer initialization", (LONGINT)48);
		Console_Ln();
		Console_String((CHAR*)"  t - don\'t check type guards (use in rare cases such as low-level modules where every cycle counts)", (LONGINT)101);
		Console_Ln();
		Console_String((CHAR*)"  P - use BasicTypeParameters file", (LONGINT)35);
		Console_Ln();
		Console_String((CHAR*)"  S - don\'t call external assembler/compiler, only generate C code", (LONGINT)67);
		Console_Ln();
		Console_String((CHAR*)"  c - don\'t call linker", (LONGINT)24);
		Console_Ln();
...
{
	__ENUMR(&OPM_inR, Texts_Reader__typ, 68, 1, P);
	P(OPM_Log);
	__ENUMR(&OPM_W, Texts_Writer__typ, 48, 1, P);
	__ENUMR(&OPM_oldSF, Files_Rider__typ, 32, 1, P);
	__ENUMR(&OPM_newSF, Files_Rider__typ, 32, 1, P);
	__ENUMR(OPM_R, Files_Rider__typ, 32, 3, P);
	P(OPM_oldSFile);
	P(OPM_newSFile);
	P(OPM_HFile);
	P(OPM_BFile);
